...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR    lpCmdLine, int nCmdShow)
{
	...
	try
	{
		...
		//12、创建渲染UI矩形（渲染阴影视图）用的管线的根签名、管线状态对象、矩形框VB、CB、采样器（堆）、SRV Heap、CBV、SRV
		{
			//--------------------------------------------------------------------------------------------------------------
			//创建渲染矩形的根签名对象
			CD3DX12_DESCRIPTOR_RANGE1 stDSPRanges[3];
			...
			CD3DX12_CPU_DESCRIPTOR_HANDLE stCPUSRVHeapQuad(pISRVHeapQuad->GetCPUDescriptorHandleForHeapStart());
			...
			stCPUSRVHeapQuad.Offset(1, g_nSRVDescriptorSize);
			...
			CD3DX12_GPU_DESCRIPTOR_HANDLE stGPUSRVHandle(pISRVHeapQuad->GetGPUDescriptorHandleForHeapStart());
			pICmdBundlesQuad->SetGraphicsRootDescriptorTable(0, stGPUSRVHandle);
			stGPUSRVHandle.Offset(1, g_nSRVDescriptorSize);
			...
			if (0 == dwRet)
			{
				...
				case EM_GRS_RS_STARTRENDER:// 状态2 阴影渲染结束 开始正常渲染
				{
					...
					//Second Pass 渲染前处理
					{
						...
						pICmdListMid->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(pIDepthShadowBuffer.Get()
							, D3D12_RESOURCE_STATE_DEPTH_WRITE
							, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE));

						//转换渲染目标状态
						pICmdListMid->ResourceBarrier(1
